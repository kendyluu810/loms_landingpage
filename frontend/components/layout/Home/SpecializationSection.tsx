...
export default function SpecializationSection() {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="mt-28 flex flex-col items-center justify-center max-w-7xl mx-auto px-4"
    >
      <h2 className="font-bold text-2xl sm:text-3xl lg:text-4xl text-center">
        Specializing in Import and Export of Goods
      </h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 sm:gap-8 mt-12 w-full">
        {cards.map((card, idx) => (
          <motion.div
            key={idx}
          ...
          >
            ...
            {/* Overlay */}
            <div className="absolute inset-0 bg-green-600 bg-opacity-60 flex flex-col items-center justify-center text-white text-center p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
