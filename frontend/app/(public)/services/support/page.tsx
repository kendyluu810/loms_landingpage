...
export default function SupportPage() {
  return (
    <div className="flex flex-col justify-center items-center mb-10">
      <HeaderBreadcrums />
      {/* support services */}
      <div className="flex flex-col lg:flex-row justify-center items-center lg:items-start gap-8 mt-28 px-6">
        {/* left */}
        <motion.div
        ...
        <motion.div
          ...
          className="w-full lg:w-1/2 h-[250px] md:h-[350px] overflow-hidden rounded-lg shadow-lg relative"
        >
          <Image
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-5">
          <div className="bg-green-600 text-white flex flex-col justify-center items-center text-center p-8 rounded-lg">
          ...
          {cards.slice(0, 2).map((item, idx) => (
            <div
              key={idx}
              className="relative h-[280px] overflow-hidden rounded-lg group"
            >
              <Image
                src={item.image}
                alt={item.title}
              ...
              <div className="absolute inset-0 bg-black/40 flex items-end justify-center transition-all duration-300 group-hover:bg-green-600/70">
                <p className="font-semibold text-lg text-white text-center p-2">
                  {item.title}
                </p>
              </div>
            </div>
          ...
          {cards.slice(2).map((item, idx) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-black/40 flex items-end justify-center transition-all duration-300 group-hover:bg-green-400/70">
                <p className="font-semibold text-base md:text-lg text-white text-center p-2">
                  {item.title}
                </p>
              </div>
            </div>
